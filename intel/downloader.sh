...
download_domain() {
    curl --max-time 30 --retry-delay 3 --retry 10 -4s -# https://raw.githubusercontent.com/$NAME_ACCOUNT_GITHUB/ipranges/main/"$1"/domains.txt
}
...
cat "${HOME_GITHUB}"/"${NAME_SERVICE}"/"${NAME_SERVICE}".json | zstd -o "${HOME_GITHUB}"/"${NAME_SERVICE}"/"${NAME_SERVICE}".json.zst && rm -f "${HOME_GITHUB}"/"${NAME_SERVICE}"/"${NAME_SERVICE}".json "${HOME_GITHUB}"/"${NAME_SERVICE}"/ipv4.txt
